...
describe('DbSubscriptionRepository unit tests', () => {
  ...
  describe('DbSubscriptionRepository.getSubscriptions', () => {
    it('returns a list of subscriptions when the limit of pagination is 1 and the page is 1', async () => {
      ...
      const { results, page_result } = await repository.getCurrentActiveSubscription({
        limit: 1,
        page: 1,
      });
      ...
      expect(page_result!.next_page).toEqual(2);
      expect(page_result!.total_of_pages).toEqual(2);
      expect(query_mock.mock.calls[0][0]).toEqual("SELECT count(s.id) as total FROM subscriptions s JOIN subscription_plans sp ON s.status = $1 AND sp.next_billing_date = $2");
      expect(query_mock.mock.calls[0][1][0]).toEqual(SubscriptionStatus.ACTIVE);
      expect(query_mock.mock.calls[0][1][1]).toBeInstanceOf(Date);
      expect(query_mock.mock.calls[1][0]).toEqual("SELECT s.id,s.subscriber_id,s.tenant_id,sp.amount FROM subscriptions s JOIN subscription_plans sp ON s.status = $1 AND sp.next_billing_date = $2 LIMIT $3 OFFSET $4");
      expect(query_mock.mock.calls[1][1][0]).toEqual(SubscriptionStatus.ACTIVE);
      expect(query_mock.mock.calls[1][1][1]).toBeInstanceOf(Date);
      expect(query_mock.mock.calls[1][1][2]).toEqual(1);
      expect(query_mock.mock.calls[1][1][3]).toEqual(0);
    });
    ...
    it('returns a list of subscriptions when the limit of pagination is 1 and the page is 2', async () => {
      ...
      expect(query_mock.mock.calls[1][1][1]).toBeInstanceOf(Date);
      expect(query_mock.mock.calls[1][1][2]).toEqual(1);
      expect(query_mock.mock.calls[1][1][3]).toEqual(1);
    });
  });
});
